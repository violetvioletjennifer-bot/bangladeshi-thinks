...
const SECRET = 'bangladeshi-thinks-secret-2024';
...
app.post('/api/signup', async (req,res) => {
  try {
    ...
    const user = {id:Date.now(),name,email,password:await bcrypt.hash(password,10),bio:'',friends:[],blocked:[]};
    db.users.push(user);
    saveDB(db);
...
app.post('/api/posts/:id/react', auth, (req,res) => {
  ...
  if (!post.reactions[req.body.type]) post.reactions[req.body.type] = 0;
...
app.listen(PORT, ()=>console.log(`🇧🇩 Bangladeshi Thinks running on port ${PORT}!`));
